...
USB HID Version — Dual Mode with Onboard Mode Selection
...
def draw_keycode_screen(index, flash_color=None, scanning=False):
    """Standard two-pane keycode display shared by both modes."""
    clear_display()
...
def draw_menu_screen():
    ...
    # Centre divider
    splash.append(create_background(WHITE, SCREEN_WIDTH // 2 - 1, 0, 2, SCREEN_HEIGHT))

    # Left half — D1 / Single Switch
    splash.append(label.Label(
        terminalio.FONT, text="D1", color=WHITE, scale=4,
        anchor_point=(0.5, 0.5),
        anchored_position=(SCREEN_WIDTH // 4, SCREEN_HEIGHT // 2 - 10)
    ))
    splash.append(label.Label(
        terminalio.FONT, text="1-SW", color=LIGHT_GRAY, scale=2,
        anchor_point=(0.5, 0.5),
        anchored_position=(SCREEN_WIDTH // 4, SCREEN_HEIGHT // 2 + 18)
    ))
    ...
    splash.append(label.Label(
        ...
        anchor_point=(0.5, 0.5),
        anchored_position=(3 * SCREEN_WIDTH // 4, SCREEN_HEIGHT // 2 - 10)
    ))
    splash.append(label.Label(
        terminalio.FONT, text="2-SW", color=LIGHT_GRAY, scale=2,
        anchor_point=(0.5, 0.5),
        anchored_position=(3 * SCREEN_WIDTH // 4, SCREEN_HEIGHT // 2 + 18)
    ))
...
print("=" * 40)
print("Switch Interface — Dual Mode")
...
while True:
    try:
        ...
        # Drain any buttons still held from a previous mode before listening
